"""
simulate constant tropical heating on a time-independent mean state. 
...
if zm:
    outfile = opath+'heating_'+config['mean_state_season']+'_zm.h5'
else:
    outfile = opath+'heating_'+config['mean_state_season']+'.h5'
...
# specify an elliptical heating pattern
logger.info('constructing the elliptical heating function...')
km = 1.e3
amp = 0.1
k = 6
ylat = 0.
xlon = 120.
locRad = 10000.*km
heating = amp*pw.perturbations_ellipse(lat,lon,k,ylat,xlon,locRad)
logger.info('max heating value (K/day): '+str(np.max(heating)))
...
# initial conditions: unperturbed mean state
...
for t in np.arange(dhr,nhours+1,dhr):
    ...
    make_steady = True
    pl_tmp,sfc_tmp = pw.run_panguweather(ort_session,1,ivp_pl_run,ivp_sfc_run,heating=heating,make_steady=make_steady,pl_dt=mean_pl_dt,sfc_dt=mean_sfc_dt)
    
    pl_last = pl_tmp[-1,:]
    sfc_last = sfc_tmp[-1,:]
    
    if np.mod(t,24)==0:
        logger.info('copying 24 hour output for the next 24 step IC...')
    ...
    if np.mod(t,ohr) == 0:
        ofile = outfile[:-3]+'_'+str(t)+'h.h5'
        logger.info('writing output to: '+ofile)
        h5f = h5py.File(ofile, 'w')
        if only_500:
